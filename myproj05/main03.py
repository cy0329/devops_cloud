...
print('"<<좋아요" 수가 200,000 이상인 곡명에 대해서, 곡명 가나다 순으로 정렬>>')
# 1안) 필터링을 먼저 하고, 필터링된 결과에 대해서 곡명 오름차순 정렬
# 2안) 곡명에 대한 오름차순 정렬을 먼저하고, 필터링

# TODO
def filter_fn4(song_dict):  # x1
    return song_dict["like"] >= 200_000  # x2
...
new_song_list = filter(filter_fn4, song_list)  # x1 # list 타입은 아니다.***중요
new_song_list = sorted(new_song_list, key=sort_fn4)  # x2
